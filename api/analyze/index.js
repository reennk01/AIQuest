export default async function (context, req) {
  ...
  if (!endpoint || !key) return { status: 500, body: { error: "Missing VISION_ENDPOINT or VISION_KEY" } };
  ...
  try {
    ...
    const denseCaptions = (json?.denseCaptionsResult?.values || []).map(dc => ({ text: dc.text, bbox: dc.boundingBox }));
